...
class DataHandler
{
    ...
    function insertComment($meetingID, $name, $comment, $termin1, $termin2)
    {
        ...
        if ($termin1 == 1) {
            $sql = "INSERT INTO kommentare (personname, comment, fk_a_id, fk_o_id) VALUES (?,'aa',2,2);";
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("s", $name, $comment, $meetingID, $null);
            $statement->execute();
            //$sql = "INSERT INTO kommentare (personname, comment, fk_a_id, fk_o_id) VALUES (?,?,?,?);";
            //$statement = $this->conn->prepare($sql);
            //$statement->bind_param("ssii", $name, $comment, $meetingID, $null);
            //$statement->execute();
        }
        ...
        if ($termin2 == 1) {
            ...
            $sql = "INSERT INTO kommentare (personname, comment, fk_a_id, fk_o_id) VALUES (?,?,?,?);";
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("ssii", $name, $comment, $meetingID, $eins);
            $statement->execute();
        }
    }
