...
class DataHandler
{
    ...
    function insertComment($meetingID, $name, $comment, $termin1, $termin2)
    {
        ...
        $select1 = "SELECT o_id FROM Options WHERE fk_a_id=? and optionsnummer=0;";
        $statement = $this->conn->prepare($select1);
        ...
        if ($termin1 == 1) {
            ...
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("ssii", $name, $comment, $meetingID, $result); //<-- muss daher
            $statement->execute();
        }

        $select2 = "SELECT o_id FROM Options WHERE fk_a_id=? and optionsnummer=1;";
        $statement = $this->conn->prepare($select2);
        $statement->bind_param("i", $meetingID);
        $statement->execute();
        $result = $statement->get_result();
        ...
        if ($termin2 == 1) {
            $sql = "INSERT INTO kommentare (personname, comment, fk_a_id, fk_o_id) VALUES (?,?,?,?);";
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("ssii", $name, $comment, $meetingID, $result);
            $statement->execute();
        }
    }
