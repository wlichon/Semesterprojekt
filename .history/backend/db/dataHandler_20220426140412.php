...
class DataHandler
{
    ...
    {
        if ($termin1 == 1) {
            $sql = "UPDATE Options SET votingcount = votingcount + 1 where meetingid=? and optionsnummer = 0;";
        }
        if ($termin2 == 2) {
            $sql = "UPDATE Options SET votingcount = votingcount + 1 where meetingid and optionsnummer = 1;";
        }
    }

    function createOptions($date, $optionbegin, $optionend, $meetingID, $optionsnummer)
    {
        $sql = "INSERT INTO Options (begin, end, fk_a_id, date, optionsnummer) VALUES (?, ?, ?, ?, ?);";
        $statement = $this->conn->prepare($sql);
        $statement->bind_param("ssisi", $optionbegin, $optionend, $meetingID, $date, $optionsnummer);
        $statement->execute();
    }
