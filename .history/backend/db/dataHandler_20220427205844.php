...
class DataHandler
{
    ...
    function insertComment($meetingID, $name, $comment, $termin1, $termin2)
    {
        ...
        if ($termin1 == 1) {
            ...
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("ssii", $name, $comment, $meetingID, $result['o_id']); //<-- muss daher
            $statement->execute();
        }

        if ($termin2 == 1) {
            $sql = "SELECT o_id FROM Options WHERE fk_a_id=100801 and optionsnummer=1;";
            $statement = $this->conn->prepare($sql);
            //$statement->bind_param("i", $meetingID);
            $statement->execute();
            $result = $statement->get_result();
            ...
            $sql = "INSERT INTO kommentare (personname, comment, fk_a_id, fk_o_id) VALUES (?,?,?,?);";
            $statement = $this->conn->prepare($sql);
            $statement->bind_param("ssii", $name, $comment, $meetingID, $result['o_id']);
            $statement->execute();
        }
    }
