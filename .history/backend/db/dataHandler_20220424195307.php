...
class DataHandler
{
    ...
    {
        $sql = "INSERT INTO Appointment (title, votingExpirationDate, beginTime, endTime, meetingDate) VALUES (?, ?, ?, ?, ?);";
        $statement = $this->conn->prepare($sql);
        $statement->bind_param("isddd", $title, $votingExpirationDate, $begin, $end, $date);
        $statement->execute();
    }
